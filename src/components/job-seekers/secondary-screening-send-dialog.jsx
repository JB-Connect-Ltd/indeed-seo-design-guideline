...
export function SecondaryScreeningSendDialog({ open, onClose, onConfirm, target }) {
  if (!open || !target) {
    return null
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-[1px]">
      <div className="w-full max-w-xl rounded-[22px] border border-border bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4">
          <div>
            <h2 className="text-lg font-semibold tracking-tight">二次書類審査フォームを送信</h2>
            <p className="mt-2 text-sm text-muted-foreground">求職者へ二次書類審査フォームURLを送信します。</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="rounded-[18px] border border-border bg-muted/20 p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-3">
                <div className="text-base font-semibold tracking-tight">このフォームURLを求職者へ送信しますか？</div>
                ...
                <p className="text-sm leading-7 text-muted-foreground">
                  求職者はURL先のフォームから二次書類審査内容を入力できます。
                </p>
              </div>
            </div>
          </div>
